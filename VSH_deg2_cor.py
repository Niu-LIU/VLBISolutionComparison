...
__all__ = ["elimination1", "wgt_mat",
           "Jac_mat_deg01", "res_arr01", "VSH_deg01", "VSHdeg01_fitting",
           "Jac_mat_deg02", "res_arr02", "VSH_deg02", "VSHdeg02_fitting",
           "test_code"]


# ------------------ FUNCTION --------------------------------
def elimination1(y1r, y2r, n=3.0):
    '''A outlier elimiantion using 3-sigma criteria.
    '''
...
def VSHdeg01_fitting(dRA, dDE, e_dRA, e_dDE, cor, RA, DE, flog):
    ...
    print("# apriori reduced Chi-square for: %10.3f" %
          calc_2Dchi2(dRA, e_dRA, dDE, e_dDE, cor, reduced=True),
          file=flog)
    ...
    while(num1 != num2):
        ...
        indice = elimination1(rRA, rDE)
        num2 = dRA.size - indice.size

        [dRAn, dDEn, e_dRAn, e_dDEn] = [np.take(dRA, indice),
                                        np.take(dDE, indice),
                                        np.take(e_dRA, indice),
                                        np.take(e_dDE, indice)]
        corn = np.take(cor, indice)
        RAn, DEn = np.take(RA, indice), np.take(DE, indice)

        wn, sign, cofn = VSH_deg01(dRAn, dDEn, e_dRAn, e_dDEn, corn,
                                   RAn, DEn)
    ...
    print("# posteriori reduced Chi-square for: %10.3f" %
          calc_2Dchi2(dRAres, e_dRA, dDEres, e_dDE, cor, reduced=True),
          file=flog)
...
def VSHdeg02_fitting(dRA, dDE, e_dRA, e_dDE, cor, RA, DE, flog):
    ...
    while(np.fabs(num2 - num1) > 1):
        ...
        # For debugging
        i += 1
        print("Iteration times: ", i, num1, num2, file=flog)
...
def test_code():
    ...
    # Using VSH degree 01.
    print('VSH deg01:')
    w, sig, corrcoef, _ = VSHdeg01_fitting(
        dRA, dDE, err1, err2, cor, RA, DE, flog)
    print('w = ', w)
    print('sigma: ', sig)
    ...
    # Using VSH degree 02.
    print('VSH deg02:')
    w, sig, corrcoef, _ = VSHdeg02_fitting(
        dRA, dDE, err1, err2, cor, RA, DE, flog)
    print('w = ', w)
    print('sigma: ', sig)
